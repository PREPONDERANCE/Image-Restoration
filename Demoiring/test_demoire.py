## Seeing the Unseen: A Frequency Prompt Guided Transformer for Image Restoration (Jittor Version - Demoireing)
...
parser = argparse.ArgumentParser(description="Image Demoireing using FPro with Jittor")
parser.add_argument(
    "--input_dir",
    default="/mnt/sda/zsh/FPro/Demoiring/test_resize286_crop256/thin_source",
    type=str,
    help="Directory of validation images",
)
parser.add_argument(
    "--result_dir", default="./results/FPro_test/", type=str, help="Directory for results"
)
parser.add_argument(
    "--weights", default="./models/demoire_noAug.pth", type=str, help="Path to weights"
)
parser.add_argument(
    "--opt",
    type=str,
    default="Options/RealDemoiring_FPro.yml",
...
def get_scoremap(H, W, C, B=1, is_mean=True):
    ...
    # Jittor change: torch.ones -> jt.ones
...
def mergeimage(split_data, starts, crop_size=128, resolution=(1, 3, 128, 128)):
    ...
    # Jittor change: torch.zeros -> jt.zeros
...
for dataset in datasets:
    ...
    with jt.no_grad():
        for file_ in tqdm(files):
            ...
            # Jittor change: nn.pad instead of F.pad
            input_ = nn.pad(input_, (0, padw, 0, padh), "reflect")

            restored = model_restoration(input_)
            restored = restored[:, :, :h, :w]

            # Jittor change: clamp, permute, squeeze. Use .numpy() instead of .cpu().detach().numpy()
