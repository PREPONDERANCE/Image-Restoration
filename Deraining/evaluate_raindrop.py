...
def evaluate_raindrop(in_dir, gt_dir):
    inputs = sorted(
        glob(os.path.join(in_dir, "*.png")) + glob(os.path.join(in_dir, "*.jpg"))
    )
    gts = sorted(
...
if __name__ == "__main__":
    ave_psnr, ave_ssim = evaluate_raindrop(
        "/mnt/sda/zsh/FPro/Deraining/results/FPro_AGAN/test_a",
        "/mnt/sda/zsh/dataset/test_a/gt",
    )
    print("PSNR: ", ave_psnr)
    print("SSIM: ", ave_ssim)
